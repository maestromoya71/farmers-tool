<?php
    ...
    if(isset($_POST['AddProduct'])){
        if(isset($_POST['username']) && (isset($_POST['phone'])) && (isset($_POST['select_category'])) && (isset($_POST['category'])) && (isset($_POST['price'])) && ($_FILES['image']['tmp_name'] == TRUE) ){
            ...
            $image = file_get_contents($image); // Read the image file as binary data
    ...
    function saveimage($name, $image){
        ...
        $select = mysqli_real_escape_string($conn, $_POST['select_category']);

        // Use prepared statements to handle binary data
        ...
        // Bind parameters
        $stmt->bind_param("sssssdss", $username, $category, $select, $description, $phone, $price, $name, $image);

        // Execute the query
        if ($stmt->execute()) {
            echo "<script>window.open('FarmerProfile.php', '_self')</script>";
        } else {
            echo "<br/>Error: " . $stmt->error; // Display the MySQL error message
        }
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
        <div class="container">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="index.php"><strong> Connect</strong></a>
            </div>
